...
!--- types needed to define CA2G_br:
 ...
 type,extends(GA_Environment),public:: CA2G_br_GridComp
    ...
    contains
       ...
       procedure:: rrtmg_GridComp     => rrtmg_CA2G_br_GridComp
...
!call mpas_log_write('--- enter subroutine DryDeposition:')
 ...
 if(associated(self%brvdep)) self%brvdep(:,:) = 0._RKIND
...
!==================================================================================================================
 subroutine rrtmg_CA2G_br_GridComp(self_params,self,its,ite,jts,jte,kts,kte,nbndlw,nbndsw)
!==================================================================================================================
!--- input arguments:
 integer,intent(in):: its,ite,jts,jte,kts,kte
...
!------------------------------------------------------------------------------------------------------------------
!call mpas_log_write(' ')
 call mpas_log_write('--- enter subroutine rrtmg_CA2G_br_GridComp:')
...
!call mpas_log_write('--- nbands = $i',intArgs=(/nbands/))
 ...
 if(associated(self%brtau_sw)) self%brtau_sw(:,:,:,:) = 0._RKIND
 if(associated(self%brasy_sw)) self%brasy_sw(:,:,:,:) = 0._RKIND
 if(associated(self%brssa_sw)) self%brssa_sw(:,:,:,:) = 0._RKIND
 if(associated(self%brtau_lw)) self%brtau_lw(:,:,:,:) = 0._RKIND
 if(associated(self%brasy_lw)) self%brasy_lw(:,:,:,:) = 0._RKIND
 if(associated(self%brssa_lw)) self%brssa_lw(:,:,:,:) = 0._RKIND
 ...
 qca2G(:,:,:,1) = self%brphobic(:,:,:)*self%delp(:,:,:)/grav
 qca2G(:,:,:,2) = self%brphilic(:,:,:)*self%delp(:,:,:)/grav
 ...
 do n = 1, nbands
    ...
    if(istat /=0) then
       call mpas_log_write('--- CA2G_br_GridComp: error in subroutine rrtmg_CA2G_br_GridComp.', &
                           messageType=MPAS_LOG_CRIT)
    else
       if(n .le. nbndsw) then
          ns = ns+1
          self%brtau_sw(:,:,:,ns) = real(ext_s(:,:,:),kind=RKIND)
          self%brasy_sw(:,:,:,ns) = real(asy_s(:,:,:),kind=RKIND)
          self%brssa_sw(:,:,:,ns) = real(ssa_s(:,:,:),kind=RKIND)
       elseif(n .gt. nbndsw) then
          nl = nl+1
          self%brtau_lw(:,:,:,ns) = real(ext_s(:,:,:),kind=RKIND)
          self%brasy_lw(:,:,:,ns) = real(asy_s(:,:,:),kind=RKIND)
          self%brssa_lw(:,:,:,ns) = real(ssa_s(:,:,:),kind=RKIND)
       endif
    endif
 enddo
 ...
 call mpas_log_write('--- end subroutine rrtmg_CA2G_br_GridComp.')
 ...
 end subroutine rrtmg_CA2G_br_GridComp
