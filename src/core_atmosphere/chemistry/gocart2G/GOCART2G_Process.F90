...
!  Only instrinsic fortran types and functions are allowed.
   ...
   use mpas_kind_types,only: RKIND,DP=>R8KIND
   use mpas_log
...
!
   ...
   real(kind=RKIND), parameter :: OCEAN=2.0, LAND = 1.0, SEA_ICE = 1.0
...
! !INPUT PARAMETERS:
   real(kind=RKIND), dimension(:), intent(in)  :: radius      ! Dry particle bin effective radius [um]
   real(kind=RKIND), dimension(:), intent(in)  :: rLow, rUp   ! Dry particle bin edge radii [um]

! !OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:), intent(out) :: distribution    ! Normalized dust aerosol distribution [1]
...
! !Local Variables
   integer :: n, nbins
   real(kind=RKIND)    :: diameter, dlam, dvol

! !CONSTANTS
   real(kind=RKIND), parameter    :: mmd    = 3.4          ! median mass diameter [um]
   real(kind=RKIND), parameter    :: stddev = 3.0          ! geometric standard deviation [1]
   real(kind=RKIND), parameter    :: lambda = 12.0         ! crack propagation length [um]
   real(kind=RKIND), parameter    :: factor = 1.e0 / (sqrt(2.e0) * log(stddev))  ! auxiliary constant
...
!
! !INTERFACE:
   real(kind=RKIND) function soilMoistureConvertVol2Grav(vsoil, sandfrac, rhop)

! !USES:
   implicit NONE

! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in) :: vsoil       ! volumetric soil moisture fraction [1]
   real(kind=RKIND), intent(in) :: sandfrac    ! fractional sand content [1]
   real(kind=RKIND), intent(in) :: rhop        ! dry dust density [kg m-3]
...
!  !Local Variables
   real(kind=RKIND) :: vsat

!  !CONSTANTS:
   real(kind=RKIND), parameter :: rhow = 1000.    ! density of water [kg m-3]
...
!
! !INTERFACE:
   real(kind=RKIND) function moistureCorrectionFecan(slc, sand, clay, rhop)

! !USES:
   implicit NONE

! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in) :: slc     ! liquid water content of top soil layer, volumetric fraction [1]
   real(kind=RKIND), intent(in) :: sand    ! fractional sand content [1]
   real(kind=RKIND), intent(in) :: clay    ! fractional clay content [1]
   real(kind=RKIND), intent(in) :: rhop    ! dry dust density [kg m-3]
...
!  !Local Variables
   real(kind=RKIND) :: grvsoilm
   real(kind=RKIND) :: drylimit
...
!
! !INTERFACE:
   real(kind=RKIND) function DustFluxV2HRatioMB95(clay, kvhmax)
...
! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in) :: clay      ! fractional clay content [1]
   real(kind=RKIND), intent(in) :: kvhmax    ! maximum flux ratio [1]

!  !CONSTANTS:
   real(kind=RKIND), parameter :: clay_thresh = 0.2    ! clay fraction above which the maximum flux ratio is returned
...
! !INPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:), intent(in) :: fraclake ! fraction of lake [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: fracsnow ! surface snow area fraction [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: slc      ! liquid water content of soil layer, volumetric fraction [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: oro      ! land-ocean-ice mask [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: clay     ! fractional clay content [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: sand     ! fractional sand content [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: silt     ! fractional silt content [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: ssm      ! erosion map [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: rdrag    ! drag partition [1/m]
   real(kind=RKIND), dimension(:,:), intent(in) :: airdens  ! air density at lowest level [kg/m^3]
   real(kind=RKIND), dimension(:,:), intent(in) :: ustar    ! friction velocity [m/sec]
   real(kind=RKIND), dimension(:,:), intent(in) :: uthrs    ! threshold velocity [m/2]
   real(kind=RKIND),                 intent(in) :: alpha    ! scaling factor [1]
   real(kind=RKIND),                 intent(in) :: gamma    ! scaling factor [1]
   real(kind=RKIND),                 intent(in) :: kvhmax   ! max. vertical to horizontal mass flux ratio [1]
   real(kind=RKIND),                 intent(in) :: grav     ! gravity [m/sec^2]
   real(kind=RKIND), dimension(:),   intent(in) :: rhop            ! soil class density [kg/m^3]
   real(kind=RKIND), dimension(:),   intent(in) :: distribution    ! normalized dust binned distribution [1]

! !OUTPUT PARAMETERS:
   real(kind=RKIND),    intent(out) :: emissions(:,:,:)     ! binned surface emissions [kg/(m^2 sec)]
...
! !Local Variables
   ...
   real(kind=RKIND)                  :: alpha_grav
   real(kind=RKIND)                  :: fracland
   real(kind=RKIND)                  :: h
   real(kind=RKIND)                  :: kvh
   real(kind=RKIND)                  :: q
   real(kind=RKIND)                  :: rustar
   real(kind=RKIND)                  :: total_emissions
   real(kind=RKIND)                  :: u_sum, u_thresh

! !CONSTANTS:
   real(kind=RKIND), parameter       :: ssm_thresh = 1.e-02    ! emit above this erodibility threshold [1]
...
! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in) :: radius(:)       ! particle radius [m]
   real(kind=RKIND), dimension(:,:), intent(in) :: fraclake ! fraction of lake [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: gwettop  ! surface soil wetness [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: oro      ! land-ocean-ice mask [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: u10m     ! 10-meter eastward wind [m/sec]
   real(kind=RKIND), dimension(:,:), intent(in) :: v10m     ! 10-meter northward wind [m/sec]
   real(kind=RKIND), dimension(:,:), intent(in) :: du_src   ! dust emissions [(sec^2 m^5)/kg]
   real(kind=RKIND), intent(in) :: Ch_DU   ! dust emission tuning coefficient [kg/(sec^2 m^5)]
   real(kind=RKIND), intent(in) :: grav    ! gravity [m/sec^2]
   
! !OUTPUT PARAMETERS:
!   real(kind=RKIND), pointer, intent(inout)  :: emissions(:,:)    ! Local emission [kg/(m^2 sec)]
   real(kind=RKIND), intent(inout)  :: emissions(:,:,:)    ! Local emission [kg/(m^2 sec)]
...
! !Local Variables
   integer         ::  i, j, n
   real(kind=RKIND), parameter ::  air_dens = 1.25  ! Air density = 1.25 kg m-3
   real(kind=RKIND), parameter ::  soil_density  = 2650.  ! km m-3 
   real(kind=RKIND)            ::  diameter         ! dust effective diameter [m]
   real(kind=RKIND)            ::  u_thresh0
   real(kind=RKIND)            ::  u_thresh
   real(kind=RKIND)            ::  w10m
   integer         ::  i1, i2, j1, j2, nbins
   integer         ::  dims(2)
!   real(kind=RKIND), allocatable ::  emissions_(:,:)
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:,:), intent(in) :: rho_air    ! air density
   real(kind=RKIND), dimension(:,:), intent(in) :: w_top      ! volumetric soil moisture in the top surface layer
   real(kind=RKIND), dimension(:,:), intent(in) :: t_soil     ! soil temperature
   real(kind=RKIND), dimension(:,:), intent(in) :: z0         ! aeolian aerodynamic roughness length
   real(kind=RKIND), dimension(:,:), intent(in) :: z, u_z, v_z! hight and wind at this height
   real(kind=RKIND), dimension(:,:), intent(in) :: ustar      ! friction velocity
   real(kind=RKIND), dimension(:,:), intent(in) :: f_land     ! land fraction
   real(kind=RKIND), dimension(:,:), intent(in) :: f_snow     ! snow fraction
   real(kind=RKIND), dimension(:,:), intent(in) :: f_src      ! dust source potential -- OBSOLETE
   real(kind=RKIND), dimension(:,:), intent(in) :: f_sand     ! sand fraction
   real(kind=RKIND), dimension(:,:), intent(in) :: f_silt     ! silt fraction
   real(kind=RKIND), dimension(:,:), intent(in) :: f_clay     ! clay fraction
   real(kind=RKIND), dimension(:,:), intent(in) :: texture    ! soil texture
   real(kind=RKIND), dimension(:,:), intent(in) :: vegetation ! vegetation categories (IGBP)
   real(kind=RKIND), dimension(:,:), intent(in) :: gvf        ! vegetation fraction

   integer, intent(in)              :: opt_clay   ! controls which clay&silt emissions term to use
   real(kind=RKIND), intent(in)                 :: Ch_DU      ! dust emission tuning coefficient [kg/(sec^2 m^5)]
   real(kind=RKIND), intent(in)                 :: f_w        ! factor to scale down soil moisture in the top 5cm to soil moisture in the top 1cm
   real(kind=RKIND), intent(in)                 :: f_c        ! scale down the wet sieving clay fraction to get it more in line with dry sieving measurements
   real(kind=RKIND), intent(in)                 :: uts_gamma  ! threshold friction velocity parameter 'gamma'
   real(kind=RKIND), intent(in)                 :: UNDEF      ! paramter for undefined varaible
   real(kind=RKIND), intent(in)                 :: GRAV       ! gravity
   real(kind=RKIND), intent(in)                 :: VON_KARMAN ! von karman constant

! !OUTPUT PARAMETERS:

   real(kind=RKIND), dimension(:,:,:), intent(out) :: emissions ! mass flux of emitted dust particles

   real(kind=RKIND), dimension(:,:), intent(out) :: u         ! aeolian friction velocity
   real(kind=RKIND), dimension(:,:), intent(out) :: u_t       ! threshold friction velocity
   real(kind=RKIND), dimension(:,:), intent(out) :: u_ts      ! threshold friction velocity over smooth surface

   real(kind=RKIND), dimension(:,:), intent(out) :: H_w       ! soil mosture correction
   real(kind=RKIND), dimension(:,:), intent(out) :: R         ! drag partition correction

   real(kind=RKIND), dimension(:,:), intent(out) :: f_erod    ! erodibility
...
!  !Local Variables
   ...
   real(kind=RKIND), dimension(:,:), allocatable :: w_g        ! gravimetric soil moisture
   real(kind=RKIND), dimension(:,:), allocatable :: w_gt       ! threshold gravimetric soil moisture

   real(kind=RKIND), dimension(:,:), allocatable :: f_veg      ! vegetation mask
   real(kind=RKIND), dimension(:,:), allocatable :: clay       ! 'corrected' clay fraction in '%'
   real(kind=RKIND), dimension(:,:), allocatable :: silt       ! 'corrected' silt fraction in '%'
   real(kind=RKIND), dimension(:,:), allocatable :: k_gamma    ! silt and clay term (gamma in K14 and I&K, 2017)
   real(kind=RKIND), dimension(:,:), allocatable :: z0s        ! smooth roughness length

   real(kind=RKIND), dimension(:,:), allocatable :: Dp_size    ! typical size of soil particles for optimal saltation
   real(kind=RKIND) :: rho_p                                   ! typical density of soil particles

   integer :: i, j, i1=1, i2, j1=1, j2, n

   real(kind=RKIND), parameter :: z0_valid = 0.08e-2      ! valid range of ARLEMS z0 is 0--0.08cm, z0 > 0.08cm is retreived but the data quality is low
   real(kind=RKIND), parameter :: z0_max = 6.25 * z0_valid! maximum roughness over arid surfaces
   real(kind=RKIND), parameter :: z0_    = 2.0e-4         ! representative aeolian aerodynamic roughness length z0 = 0.02cm

   real(kind=RKIND), parameter :: rho_water     = 1000.0  ! water density, 'kg m-3'
   real(kind=RKIND), parameter :: rho_soil_bulk = 1700.0  ! soil bulk density, 'kg m-3'
   real(kind=RKIND), parameter :: rho_soil      = 2500.0  ! soil particle density, 'kg m-3'

!  real(kind=RKIND), parameter :: f_w = 0.5               ! factor to scale down soil moisture in the top 5cm to soil moisture in the top 1cm
!  real(kind=RKIND), parameter :: f_c = 0.7               ! scale down the wet sieving clay fraction to get it more in line with dry sieving measurements

   ! Shao et al.
   real(kind=RKIND), parameter :: a_n = 0.0123
   real(kind=RKIND), parameter :: G   = 1.65e-4

   ! size of coarsest mode in the STATSGO/FAO soil type
   real(kind=RKIND), parameter :: Dc_soil(12) = (/ 710e-6, 710e-6, 125e-6, &
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:,:), intent(in) :: u           ! friction velocity, 'm s-1'
   real(kind=RKIND), dimension(:,:), intent(in) :: u_t         ! threshold friction velocity, 'm s-1'
   real(kind=RKIND), dimension(:,:), intent(in) :: rho_air     ! air density, 'kg m-3'
   real(kind=RKIND), dimension(:,:), intent(in) :: f_erod      ! erodibility
   real(kind=RKIND), dimension(:,:), intent(in) :: k_gamma     ! clay and silt dependent term that modulates the emissions

! !OUTPUT PARAMETERS:
   real(kind=RKIND), intent(out)    :: emissions(:,:)          ! total vertical dust mass flux, 'kg m-2 s-1'

   character(len=*), parameter :: myname = 'VerticalDustFluxK14'

! !Local Variables
   integer :: i, j
   real(kind=RKIND)    :: u_st                     ! standardized threshold friction velocity
   real(kind=RKIND)    :: C_d                      ! dust emission coefficient
   real(kind=RKIND)    :: f_ust                    ! numerical term

  ! parameters from Kok et al. (2012, 2014)
   real(kind=RKIND), parameter :: rho_a0 = 1.225   ! standard atmospheric density at sea level, 'kg m-3'
   real(kind=RKIND), parameter :: u_st0  = 0.16    ! the minimal value of u* for an optimally erodible soil, 'm s-1'
   real(kind=RKIND), parameter :: C_d0   = 4.4e-5  ! C_d0 = (4.4 +/- 0.5)*1e-5
   real(kind=RKIND), parameter :: C_e    = 2.0     ! C_e  = 2.0 +/- 0.3
   real(kind=RKIND), parameter :: C_a    = 2.7     ! C_a  = 2.7 +/- 1.0
...
!   !ARGUMENTS:
    ...
    real(kind=RKIND), dimension(:),     intent(in)  :: pBase  ! base altitude (e.g., bottom of plume)
    real(kind=RKIND), dimension(:),     intent(in)  :: pTop   ! top altitude (e.g., top of plume)
    real(kind=RKIND), dimension(:),     intent(in)  :: pEmis  ! emission flux (e.g., kg/sec of species)
    ...
    real(kind=RKIND), dimension(:,:,:), intent(in)  :: hghte  ! model level geopotential height [m]
    real(kind=RKIND), dimension(:,:),   intent(in)  :: area   ! grid cell area [m^2]
    ...
    real(kind=RKIND), dimension(:,:,:), intent(inout)  :: emissions_point ![kg/kg]
    integer, optional,      intent(out)  :: rc                ! return code

!   !Local
    real(kind=RKIND), dimension(km)              :: point_column_emissions
    integer                          :: n, i, j
    real(kind=RKIND), dimension(:), allocatable  :: pEmis_
...
! !INPUT PARAMETERS:
   integer,            intent(in)  :: km       ! total model levels
   real(kind=RKIND), dimension(:), intent(in)  :: hghte    ! model level geopotential height [m]
   real(kind=RKIND),               intent(in)  :: z_bot, z_top ! base and top altitude respectively
   real(kind=RKIND),               intent(in)  :: area     ! grid cell area [m^2]
   real(kind=RKIND),               intent(in)  :: emissions_point ![kg/kg]


! !OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:), intent(out) ::  point_column_emissions ![kg/kg]
...
! !Locals
   ...
   integer :: k_bot, k_top
   real(kind=RKIND)    :: z_
   real(kind=RKIND), dimension(km) :: z, dz, w_ !dz units = meters
...
!INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in) :: cdt        ! time-step [s]
   real(kind=RKIND), intent(in) :: grav       ! gravity [m/sec^2]
   real(kind=RKIND), intent(in) :: radiusInp  ! particle radius [microns]
   real(kind=RKIND), intent(in) :: rhopInp    ! soil class density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: tmpu     ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rhoa     ! air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rh       ! relative humidity [1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: hghte    ! geopotential height [m]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: delp     ! pressure level thickness [Pa]

   real(kind=RKIND), dimension(:,:,:), intent(inout) :: int_qa         ! aerosol [kg/kg]

! !OUTPUT PARAMETERS:

   real(kind=RKIND), pointer, dimension(:,:), intent(inout) :: fluxout ! mass lost by settling
                                                                       ! to surface, kg/m2/s
   integer, optional, intent(out)             :: rc                    ! Error return code:
                                                                       !  0 - all is well
                                                                       !  1 -
!  Optionally output the settling velocity calculated
   real(kind=RKIND), pointer, optional, dimension(:,:,:)  :: vsettleOut
...
!  parameters from Maring et al, 2003
   real(kind=RKIND), parameter :: v_upwardMaring = 0.33e-2   ! upward velocity, [m s-1]
   real(kind=RKIND), parameter :: diameterMaring = 7.30e-6   ! particle diameter, [m]

   integer         :: i1=1, i2, j1=1, j2
   integer         :: dims(3)

   real(kind=RKIND), pointer, dimension(:,:)   :: hsurf
   real(kind=RKIND), allocatable    :: dz(:,:,:)
   real(kind=RKIND), dimension(:,:,:), allocatable  :: radius, rhop, qa
   real(kind=RKIND), dimension(:,:,:), allocatable  :: vsettle   ! fall speed [m s-1]
   real(kind=RKIND) ::  ONE_OVER_G
...
!-------------------------------------------------------------------------
!call mpas_log_write(' ')
!call mpas_log_write('--- enter subroutine Chem_SettlingSimple:')
!call mpas_log_write('--- km  = $i',intArgs=(/km/))
!call mpas_log_write('-- klid = $i',intArgs=(/klid/))
...
!--- begin LDF: this section of the sourcecode does not seem to work for MPAS. simply calculate
!    the layer thickness using hghte:
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: radius              ! Particle radius [m]
   real(kind=RKIND), intent(in)    :: rhop                ! Particle density [kg m-3]
   real(kind=RKIND), intent(in)    :: rhoa                ! Layer air density [kg m-3]
   real(kind=RKIND), intent(in)    :: tmpu                ! Layer temperature [K]
   real(kind=RKIND), intent(in)    :: grav                ! Gravity [m s-2]

! !OUTPUT PARAMETERS:

   real(kind=RKIND), intent(out)   :: vsettle                 ! Layer fall speed [m s-1]
...
! !Local Variables
   real(kind=RKIND) :: rmu                           ! Dynamic viscosity [kg m-1 s-1]
   real(kind=RKIND) :: vt                            ! Thermal velocity of air molecule [m s-1]
   real(kind=RKIND) :: rmfp                          ! Air molecule mean free path [m]
   real(kind=RKIND) :: bpm                           ! Cunningham slip correction factor
   real(kind=RKIND) :: rkn                           ! Knudsen number
   real(kind=RKIND) :: re, x, y                      ! reynold's number and parameters
   real(kind=RKIND), parameter :: kb = 1.3807e-23    ! Boltzmann constant [kg m2 s-1 K-1 mol-1]
   real(kind=RKIND), parameter :: m_air = 4.8096e-26 ! Mass of <avg> air molecule [kg]
   real(kind=RKIND), parameter :: pi = 3.141529265

   real(kind=RKIND), parameter :: f_vt = 8*kb/pi/m_air
   real(kind=RKIND), parameter :: f_diff_coef = kb/(6*pi)
   real(kind=RKIND), parameter :: two_over_nine = 2./9.

   real(kind=RKIND), parameter :: a0 = -3.18657
   real(kind=RKIND), parameter :: a1 =  0.992696
   real(kind=RKIND), parameter :: a2 = -1.53193e-3
   real(kind=RKIND), parameter :: a3 = -9.870593e-4
   real(kind=RKIND), parameter :: a4 = -5.78878e-4
   real(kind=RKIND), parameter :: a5 =  8.55176e-5
   real(kind=RKIND), parameter :: a6 = -3.27815e-6
...
! !IROUTINE: SettlingSolver
  ...
  subroutine SettlingSolver(i1, i2, j1, j2, km, cdt, delp, dz, vs, qa)
    ...
    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(in) :: delp
    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(in) :: dz
    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(in) :: vs

    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(inout) :: qa
    ...
    real(kind=RKIND), dimension(i1:i2, j1:j2, km) :: tau

    real(kind=RKIND), dimension(km) :: dp_
    real(kind=RKIND), dimension(km) :: tau_

    real(kind=RKIND) :: dt, dt_cfl
...
! !IROUTINE: ParticleSwelling
   ...
   subroutine ParticleSwelling (i1, i2, j1, j2, km, rh, radius_dry, rhop_dry, radius, rhop, flag)
    ...
    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(in)  :: rh

    integer, intent(in) :: flag

    real(kind=RKIND), intent(in) :: radius_dry
    real(kind=RKIND), intent(in) :: rhop_dry

    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(out) :: radius  ! radius  of the wet particle
    real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(out) :: rhop    ! density of the wet particle
   ...
   subroutine ParticleSwelling_Fitzgerald(i1, i2, j1, j2, km, rh, radius_dry, rhop_dry, radius, rhop)
    ...
    real(kind=RKIND), parameter :: epsilon = 1.   ! soluble fraction of deliqeuscing particle
    real(kind=RKIND), parameter :: alphaNaCl = 1.35

    real(kind=RKIND) :: alpha, alpha1, alpharat, beta, theta, f1, f2
    real(kind=RKIND) :: sat, rrat
...
! instead, it is faster to do
   ...
   subroutine ParticleSwelling_Gerber(i1, i2, j1, j2, km, rh, radius_dry, rhop_dry, radius, rhop)
    ...
    real(kind=RKIND), parameter :: c1=0.7674, c2=3.079, c3=2.573e-11, c4=-1.424

    real(kind=RKIND) :: sat, rrat, rcm
   ...
   subroutine ParticleSwelling_Gerber_AmmoniumSulfate(i1, i2, j1, j2, km, rh, radius_dry, rhop_dry, radius, rhop)
    ...
    real(kind=RKIND), parameter :: SU_c1=0.4809, SU_c2=3.082, SU_c3=3.110e-11, SU_c4=-1.428
...
! !INPUT PARAMETERS:
   ...
   integer, intent(in) :: bin    ! aerosol bin index
   real(kind=RKIND), intent(in) :: grav        ! gravity [m/sec^2]
   real(kind=RKIND), intent(in) :: cdt         ! chemistry model time-step
   real(kind=RKIND), intent(in)  :: radiusInp  ! particle radius [microns]
   real(kind=RKIND), intent(in)  :: rhopInp    ! soil class density [kg/m^3]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: int_qa  ! aerosol [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: tmpu   ! temperature [K]
...
! !OUTPUT PARAMETERS:
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: fluxout ! Mass lost by settling to surface [kg/(m^2 sec)]

!  Optionally output the settling velocity calculated
   real(kind=RKIND), dimension(:,:,:), optional, intent(out)  :: vsettleOut !Layer fall speed [m/sec]
...
!  parameters from Maring et al, 2003
   ...
   real(kind=RKIND), allocatable    :: dz(:,:,:)
   real(kind=RKIND) :: radius, rhop   ! particle radius and density passed to
                          ! fall velocity calculation
   real(kind=RKIND) :: minTime, qmin
   real(kind=RKIND) :: sat, rrat
   real(kind=RKIND) :: alpha, alpha1, alpharat, beta, theta, f1, f2
   real(kind=RKIND) :: rcm
   real(kind=RKIND) :: diff_coef                 ! Brownian diffusion coefficient [m2 s-1]
...
!-------------------------------------------------------------------------
!call mpas_log_write(' ')
!call mpas_log_write('--- enter subroutine Chem_Settling2Gorig:')
!call mpas_log_write('--- km   = $i',intArgs=(/km/))
!call mpas_log_write('--- klid = $i',intArgs=(/klid/))
...
! !OUTPUT PARAMETERS:
   real(kind=RKIND), intent(out)   :: diff_coef  ! Brownian diffusion
                                                 ! coefficient [m2/sec]
   real(kind=RKIND), intent(out)   :: vsettle    ! Layer fall speed [m s-1]
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)        :: radiusInp ! aerosol radius  microns
   real(kind=RKIND), intent(in)        :: rhopInp   ! aerosol density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: int_qa ! aerosol [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: tmpu ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rhoa  ! air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rh    ! relative humidyt [1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: hghte ! geopotential height [m]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: delp  ! air pressure thickness [Pa]
   logical, optional, intent(in)    :: correctionMaring

! !OUTPUT PARAMETERS:
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)  :: fluxout
   real(kind=RKIND), dimension(:,:,:), optional, intent(out)  :: vsettleOut
   integer, optional, intent(out)   :: rc
...
!  Begin
!call mpas_log_write(' ')
!call mpas_log_write('--- enter subroutine Chem_SettlingSimpleOrig:')
!call mpas_log_write('--- km   = $i',intArgs=(/km/))
!call mpas_log_write('--- klid = $i',intArgs=(/klid/))
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: tmpu    ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rhoa    ! air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: hghte   ! top of layer geopotential height [m]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: oro     ! orography flag
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: ustar   ! friction speed [m/sec]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: pblh    ! PBL height [m]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: shflux  ! sfc. sens. heat flux [W m-2]
   real(kind=RKIND), intent(in)                :: von_karman ! Von Karman constant [unitless]
   real(kind=RKIND), intent(in)                :: cpd       ! thermodynamic constant, specific heat of something?
   real(kind=RKIND), intent(in)                :: grav      ! gravity [m/sec^2]
   real(kind=RKIND), pointer, dimension(:,:)   :: z0h       ! rough height, sens. heat [m]

! !OUTPUT PARAMETERS:
   real(kind=RKIND), intent(inout)        :: drydepf(:,:)     ! Deposition frequency [1/sec]
   integer, intent(out)          :: rc          ! Error return code:

! !OPTIONAL PARAMETERS:
!  If these parameters are provided we compute a "resuspension" term as
!  if the particles are lifted like dust
   real(kind=RKIND), optional                            :: radius    ! particle radius [m]
   real(kind=RKIND), optional                            :: rhop      ! particle density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:), optional   :: u10m      ! 10-m u-wind component [m/sec]
   real(kind=RKIND), pointer, dimension(:,:), optional   :: v10m      ! 10-m v-wind component [m/sec]
   real(kind=RKIND), pointer, dimension(:,:), optional   :: fraclake  ! fraction covered by water [1]
   real(kind=RKIND), pointer, dimension(:,:), optional   :: gwettop   ! fraction soil moisture [1]
...
! !Local Variables
   ...
   integer :: i1=1, i2, j1=1, j2
   real(kind=RKIND), parameter :: rhow = 1000.      ! density of water [kg m-3]
   real(kind=RKIND), parameter :: coll_size = 0.002 ! collector size [m]
   real(kind=RKIND), allocatable :: dz(:,:)     ! lowest layer thickness
   real(kind=RKIND), allocatable :: rmu(:,:)    ! dynamic viscosity [kg m-1 s-1]
   real(kind=RKIND), allocatable :: Ra(:,:)     ! aerodynamic resistance
   real(kind=RKIND), allocatable :: Rs(:,:)     ! surface resistance
   real(kind=RKIND), allocatable :: vdep(:,:)   ! Deposition speed [m s-1]
   real(kind=RKIND), allocatable :: obk(:,:)    ! Obukhov Length [m]
...
!--- begin LDF: correction to thickness of layer adjacent to surface:
   dz = hghte(:,:,km) - hghte(:,:,km+1)
!--- end LDF.
...
!  ----------------------------
   do j = j1, j2
    do i = i1, i2
      ...
      z0h_ = max ( real(z0h(i,j), kind=DP), z0h_min )
...
!     Set to small value of vds if ustar too small
      vds = max(vds, real(0.002 * 0.00001, kind=DP))
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in) :: cpd        ! thermodynamic constant, specific heat of something?
   real(kind=RKIND), intent(in) :: grav       ! gravity [m/sec^2]
   real(kind=RKIND), dimension(i1:i2,j1:j2)  :: t         ! temperature [K]
   real(kind=RKIND), dimension(i1:i2,j1:j2)  :: rhoa      ! air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:) :: ustar     ! friction speed [m/sec]
   real(kind=RKIND), pointer, dimension(:,:) :: shflux    ! sfc. sens. heat flux [W/m^2]

! !OUTPUT PARAMETERS
   real(kind=RKIND), dimension(i1:i2,j1:j2)  :: obk       ! Obukhov length [m]
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: grav    ! gravity [m/sec^2]
   real(kind=RKIND), intent(in)    :: fwet
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: aerosol  ! internal state aerosol [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: ple     ! pressure level thickness [Pa]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: tmpu    ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: rhoa    ! moist air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: pfllsan ! 3D flux of liquid nonconvective precipitation [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: pfilsan ! 3D flux of ice nonconvective precipitation [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: precc   ! surface convective rain flux [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: precl   ! Non-convective precipitation [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:,:)  :: fluxout ! tracer loss flux [kg m-2 s-1]
...
! !Local Variables
   ...
   real(kind=RKIND), allocatable, dimension(:,:,:) :: pdog   ! air mass factor dp/g [kg m-2]
   real(kind=RKIND), allocatable, dimension(:,:,:) :: delz   ! box height  dp/g/rhoa [m]
   real(kind=RKIND) :: pls, pcv, pac             ! ls, cv, tot precip [mm day-1]
   real(kind=RKIND) :: qls(km), qcv(km)          ! ls, cv portion of moisture tendency[kg m-3 s-1]
   real(kind=RKIND) :: qmx, qd, A                ! temporary variables on moisture
   real(kind=RKIND) :: F, B, BT                  ! temporary variables on cloud, freq.
   real(kind=RKIND) :: WASHFRAC, WASHFRAC_F_14
   real(kind=RKIND), allocatable :: fd(:,:)      ! flux across layers [kg m-2]
   real(kind=RKIND), allocatable :: dpfli(:,:,:)  ! vertical gradient of LS ice+rain precip flux
   real(kind=RKIND), allocatable :: DC(:)        ! scavenge change in mass mixing ratio
   real(kind=RKIND), allocatable, dimension(:,:,:) :: c_h2o, cldliq, cldice

!  Rain parameters from Liu et al.
   real(kind=RKIND), parameter :: B0_ls = 1.0e-4
   real(kind=RKIND), parameter :: F0_ls = 1.0
   real(kind=RKIND), parameter :: XL_ls = 5.0e-4
   real(kind=RKIND), parameter :: B0_cv = 1.5e-3
   real(kind=RKIND), parameter :: F0_cv = 0.3
   real(kind=RKIND), parameter :: XL_cv = 2.0e-3
   real(kind=RKIND), parameter :: k_wash = 1.d0  ! first order washout rate, constant, [cm^-1]
!  Duration of rain: ls = model timestep, cv = 1800 s (<= cdt)
   real(kind=RKIND)            :: Td_ls
   real(kind=RKIND), parameter :: Td_cv = 1800.
...
!--- begin LDF: this section of the sourcecode does not work with MPAS because of the indexing of the
!    array ple: 
!  pdog = (ple(:,:,1:km)-ple(:,:,0:km-1)) / grav
!  delz = pdog / rhoa
!  dpfli = pfllsan(:,:,1:km)-pfllsan(:,:,0:km-1)+pfilsan(:,:,1:km)-pfilsan(:,:,0:km-1)

   pdog = (ple(:,:,2:km+1)-ple(:,:,1:km)) / grav
   delz = pdog / rhoa
   dpfli = pfllsan(:,:,:) + pfilsan(:,:,:)
!--- end LDF.
...
! !INPUT PARAMETERS:
!   real(kind=RKIND), pointer, dimension(:,:)     :: emissions_surface
   real(kind=RKIND), dimension(:,:,:), intent(in)     :: emissions_surface
   real(kind=RKIND), dimension(:,:,:,:), intent(inout) :: emissions
   real(kind=RKIND), dimension(:,:,:), intent(in) :: emissions_point

   real(kind=RKIND), dimension(:), intent(in)  :: sfrac ! source fraction [1]
   integer, intent(in)             :: nPts  ! number of point emissions
   integer, intent(in)             :: km    ! total model levels
   real(kind=RKIND), intent(in)                :: cdt   ! chemistry model time-step [sec]
   real(kind=RKIND), intent(in)                :: grav  ! gravity [m/sec^2]
   integer, intent(in)             :: nbins ! number of aerosol size bins
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: delp  ! pressure thickness [Pa]
   real(kind=RKIND), pointer, dimension(:,:,:,:), intent(inout)  :: aero ! aerosol [kg/kg]
...
! !INPUT PARAMETERS:
   ...
   integer,    intent(in)    :: klid   ! index for pressure lid
   real(kind=RKIND), optional, dimension(:), intent(in)    :: rlow   ! bin radii - low bounds
   real(kind=RKIND), optional, dimension(:), intent(in)    :: rup    ! bin radii - upper bounds
   real(kind=RKIND), dimension(:), intent(in)    :: wavelengths_profile
   real(kind=RKIND), dimension(:), intent(in)    :: wavelengths_vertint
   real(kind=RKIND), dimension(:,:,:,:), intent(in) :: aerosol     !
   real(kind=RKIND), intent(in) :: grav
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: tmpu  ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rhoa  ! air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: delp  ! pressure thickness [Pa]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rh    ! relative humidity [1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: u     ! east-west wind [m/s]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: v     ! north-south wind [m/s]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: ple   ! level edge air pressure [Pa]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: tropp ! tropopause pressure [Pa]
   logical, optional, intent(in)               :: NO3nFlag

! !OUTPUT PARAMETERS:
!  Total mass
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: sfcmass   ! sfc mass concentration kg/m3
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: colmass   ! col mass density kg/m2
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout) :: mass      ! 3d mass mixing ratio kg/kg
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout) :: conc      ! 3d mass concentration, kg/m3
!  Total optical properties
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: exttau    ! ext. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: stexttau  ! stratospheric ext. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: scatau    ! sct. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: stscatau  ! stratospheric sct. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: sfcmass25 ! sfc mass concentration kg/m3 (pm2.5)
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: colmass25 ! col mass density kg/m2 (pm2.5)
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout) :: mass25    ! 3d mass mixing ratio kg/kg (pm2.5)
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: exttau25  ! ext. AOT at 550 nm (pm2.5)
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: scatau25  ! sct. AOT at 550 nm (pm2.5)
   real(kind=RKIND), optional, dimension(:,:),  intent(inout)  :: aerindx   ! TOMS UV AI
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: fluxu     ! Column mass flux in x direction
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: fluxv     ! Column mass flux in y direction
   real(kind=RKIND), optional, dimension(:,:,:,:), intent(inout) :: extcoef   ! 3d ext. coefficient, 1/m
   real(kind=RKIND), optional, dimension(:,:,:,:), intent(inout) :: scacoef   ! 3d scat.coefficient, 1/m
   real(kind=RKIND), optional, dimension(:,:,:,:), intent(inout) :: bckcoef   ! 3d backscatter coefficient, m-1 sr-1
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: exttaufm  ! fine mode (sub-micron) ext. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)   :: scataufm  ! fine mode (sub-micron) sct. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:), intent(inout)   :: angstrom  ! 470-870 nm Angstrom parameter
   integer, optional, intent(out)   :: rc        ! Error return code:
                                                 !  0 - all is well
                                                 !  1 -
...
! !Local Variables
   ...
   real(kind=RKIND), allocatable, dimension(:,:,:) :: tau, ssa, bck
!  real(kind=RKIND) :: fPMfm(nbins)  ! fraction of bin with particles diameter < 1.0 um
!  real(kind=RKIND) :: fPM25(nbins)  ! fraction of bin with particles diameter < 2.5 um
   real(kind=RKIND), dimension(:), allocatable :: fPMfm  ! fraction of bin with particles diameter < 1.0 um
   real(kind=RKIND), dimension(:), allocatable :: fPM25  ! fraction of bin with particles diameter < 2.5 um
   logical :: do_angstrom
   real(kind=RKIND), dimension(:,:), allocatable :: tau470, tau870
...
! !INPUT/OUTPUT PARAMETERS:
  real(kind=RKIND), dimension(:), intent(inout) :: fPM     ! bin-wise PM fraction (r < rPM)

! !INPUT PARAMETERS:
   real(kind=RKIND),    intent(in)              :: rPM     ! PM radius
   integer, intent(in)                          :: nbins   ! number of bins
   real(kind=RKIND), dimension(:), intent(in)   :: r_low   ! bin radii - low bounds
   real(kind=RKIND), dimension(:), intent(in)   :: r_up    ! bin radii - upper bounds
...
! !INPUT/OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:), intent(inout) :: deep_lakes_mask

! !INPUT PARAMETERS:
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: lats ! latitude [radians]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: lons ! longtitude [radians]
...
! !INPUT/OUTPUT PARAMETERS:
  real(kind=RKIND), dimension(:,:), intent(inout) :: fsstemis     !

! !INPUT PARAMETERS:
   integer, intent(in)                       :: sstEmisFlag  ! 1 or 2
   real(kind=RKIND), dimension(:,:), intent(in)          :: ts  ! surface temperature (K)
...
! !Local Variables
   real(kind=RKIND), allocatable, dimension(:,:) :: tskin_c
!EOP
!-------------------------------------------------------------------------
!  Begin...
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:,:), intent(in)       :: u10m
   real(kind=RKIND), dimension(:,:), intent(in)       :: v10m
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)             :: rLow, rUp   ! Dry particle bin edge radii [um]
   real(kind=RKIND), intent(in)             :: u10m(:,:)   ! 10-meter eastward wind [m s-1]
   real(kind=RKIND), intent(in)             :: v10m(:,:)   ! 10-m northward wind [m s-1]
   real(kind=RKIND), target, intent(in)     :: ustar(:,:)  ! friction velocity [m s-1]
   integer, intent(in)                      :: method      ! Algorithm to use
   real(kind=RKIND), intent(in)             :: pi          ! pi constant

! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:), intent(inout) :: memissions      ! Mass Emissions Flux [kg m-2 s-1]
   real(kind=RKIND), dimension(:,:), intent(inout) :: nemissions      ! Number Emissions Flux [# m-2 s-1]
...
! !Local Variables
   integer       :: ir
   real(kind=RKIND), pointer :: w(:,:)                          ! Intermediary wind speed [m s-1]
   real(kind=RKIND)          :: r, dr                           ! sub-bin radius spacing (dry, um)
   real(kind=RKIND)          :: rwet, drwet                     ! sub-bin radius spacing (rh=80%, um)
   real(kind=RKIND)          :: aFac, bFac, scalefac, rpow, exppow, wpow
   real(kind=RKIND), allocatable, dimension(:,:), target  :: w10m  ! 10-m wind speed [m s-1]

! !CONSTANTS
   real(kind=RKIND), parameter    :: r80fac = 1.65     ! ratio of radius(RH=0.8)/radius(RH=0.) [Gerber]
   real(kind=RKIND), parameter    :: rhop = 2200.      ! dry seasalt density [kg m-3]
!   real(kind=RKIND), parameter    :: pi = 3.1415      ! ratio of circumference to diameter of circle
...
! where r is the particle radius at 80% RH, dr is the size bin width at 80% RH, and w is the wind speed
  ...
  function SeasaltEmissionGong ( r, dr, w, scalefac, aFac, bFac, rpow, exppow, wpow )
   ...
   real(kind=RKIND), intent(in)    :: r, dr     ! Wet particle radius, bin width [um]
   real(kind=RKIND), pointer, intent(in)    :: w(:,:)    ! Grid box mean wind speed [m s-1] (10-m or ustar wind)
   real(kind=RKIND), intent(in)    :: scalefac, aFac, bFac, rpow, exppow, wpow
   real(kind=RKIND)                :: SeasaltEmissionGong(size(w,1),size(w,2))
...
! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in)  :: radius    ! dry radius [m]
   real(kind=RKIND), intent(in)  :: rhop      ! dry density [kg m-3]
   real(kind=RKIND), intent(in)  :: rh        ! relative humidity [0-1]
   integer                       :: flag      ! 1 (Fitzgerald, 1975)
                                              ! 2 (Gerber, 1985)

! !OUTPUT PARAMETERS:
   real(kind=RKIND), intent(out) :: radius_wet ! humidified radius [m]
   real(kind=RKIND), intent(out) :: rhop_wet   ! wet density [kg m-3]
   integer, intent(out) :: rc

! !Local Variables
   real(kind=RKIND) :: sat, rcm, rrat
   real(kind=RKIND), parameter ::  rhow = 1000.  ! Density of water [kg m-3]
...
! !INPUT PARAMETERS:
   ...
   integer, intent(in)  :: rhFlag                ! 1 (Fitzgerald, 1975)
                                                 ! 2 (Gerber, 1985)
   real(kind=RKIND), dimension(:,:), intent(in)  :: rh    ! relative humidity [0-1]
   real(kind=RKIND), dimension(:,:), intent(in)  :: dz    ! surface layer height [m]
   real(kind=RKIND), dimension(:,:), intent(in)  :: ustar ! surface velocity scale [m s-1]
   real(kind=RKIND), dimension(:,:), intent(in)  :: airdens ! air density [kg/m^3]s
   real(kind=RKIND), dimension(:,:), intent(in)  :: t  ! temperature [k]
   real(kind=RKIND), intent(in)  :: grav    ! gravity [m/sec^2]
   real(kind=RKIND), intent(in)  :: karman  ! Von Karman constant [unitless]


! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:), intent(inout) :: fhoppel

! !OUTPUT PARAMETERS:
   integer, intent(out) :: rc

! !Local Variables
   real(kind=RKIND)    :: radius_wet ! humidified radius [m]
   real(kind=RKIND)    :: rhop_wet   ! wet density [kg m-3]
   real(kind=RKIND)    :: diff_coef
   real(kind=RKIND), allocatable, dimension(:,:) ::  vsettle
   integer :: i, j
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: cdt    ! chemistry model time-step [sec]
   real(kind=RKIND), intent(in)    :: grav   ! gravity [m/sec^2]
   character(len=2), intent(in)    :: prefix ! variable name prefix
   real(kind=RKIND), intent(in)    :: ratPOM
   real(kind=RKIND), intent(in)    :: eAircraftFuel ! Aircraft emission factor: go from kg fuel to kg C
   real(kind=RKIND), dimension(:), intent(in)  :: aviation_layers ! Heights [m] of LTO, CDS and CRS aviation emissions layers
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: pblh  ! PBL height [m]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: tmpu  ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: rhoa  ! air density [kg m-3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: rh    ! relative humidity [1]
   ...
   real(kind=RKIND), dimension(:,:), intent(in) :: aviation_cds_src ! Climb/Descent aircraft fuel emission [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: aviation_crs_src ! Cruise aircraft fuel emission [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: aviation_lto_src ! Landing/Take-off aircraft fuel emission [1]
   real(kind=RKIND), dimension(:,:), intent(in) :: biomass_src
   real(kind=RKIND), dimension(:,:), intent(in) :: terpene_src
   real(kind=RKIND), dimension(:,:), intent(in) :: eocant1_src  ! anthropogenic emissions
   real(kind=RKIND), dimension(:,:), intent(in) :: eocant2_src  ! anthropogenic emissions
   real(kind=RKIND), dimension(:,:), intent(in) :: oc_ship_src  ! ship emissions
   real(kind=RKIND), dimension(:,:), intent(in) :: biofuel_src  ! biofuel emissions
   real(kind=RKIND), intent(in) :: fHydrophobic

! !OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: aerosolPhobic
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: aerosolPhilic
   real(kind=RKIND), pointer, dimension(:,:,:) :: OC_emis    ! OC emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)   :: OC_emisAN  ! OC emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)   :: OC_emisBB  ! OC emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)   :: OC_emisBF  ! OC emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)   :: OC_emisBG  ! OC emissions, kg/m2/s
   integer, optional, intent(out) :: rc                      ! Error return code:
                                                             !  0 - all is well
                                                             !  1 -
...
!  pressure at 100m, 500m, & PBLH
   ...
   real(kind=RKIND), dimension(:,:), allocatable :: p0, z0, ps
   real(kind=RKIND) :: p1, z1, dz, delz, delp_, f100, f500, fPBL, fBot
   real(kind=RKIND) :: qmax, qmin, eBiofuel, eBiomass, eTerpene, eAnthro

   real(kind=RKIND), dimension(:,:), allocatable :: factor, srcHydrophobic, srcHydrophilic
   real(kind=RKIND), dimension(:,:), allocatable :: srcBiofuel, srcBiomass, srcAnthro, srcBiogenic
   real(kind=RKIND)                              :: srcTmp, zpbl, maxAll

   real(kind=RKIND), dimension(:,:,:), allocatable :: emis_aviation
   real(kind=RKIND), dimension(:,:,:), allocatable :: srcAviation
   real(kind=RKIND)                            :: z_lto_bot, z_lto_top
   real(kind=RKIND)                            :: z_cds_bot, z_cds_top
   real(kind=RKIND)                            :: z_crs_bot, z_crs_top

   real(kind=RKIND), dimension(:,:), allocatable          :: f_bb_        ! scaling factor for BB emissions based on maximum allowed exttau
   real(kind=RKIND), dimension(:,:), allocatable          :: exttau_bb_   ! increment of exttau due to BB during the current time step
   real(kind=RKIND), allocatable, dimension(:,:,:,:)      :: qa_bb_       ! increment of qa due to BB during the current time step (nbins,i1:i2,j1:j2:km) 
                                                                          ! W.Jiang note, changed to (i1:i2,j1:j2,km,nbins) for efficiency
   real(kind=RKIND)                      :: cutoff_bb_exttau
   integer                               :: idx
   integer                               :: ilam550, status
   real(kind=RKIND)                      :: wavelength550
   real(kind=RKIND), dimension(:,:,:), allocatable   :: tau
   character(len=255)                    :: qname
   real(kind=RKIND), parameter           :: max_bb_exttau = 30.0

!  Indices for point emissions
   real(kind=RKIND), dimension(km)       :: point_column_emissions

!  Source function terms for SOA from Anthropogenic VOCs
   real(kind=RKIND) :: srcSOAanthro = 0.0
!  Initialize local variables
!  --------------------------
   i2 = size(rhoa,1)
   j2 = size(rhoa,2)
...
!   --------------------------------
   ...
   subroutine distribute_aviation_emissions(delp, rhoa, z_bot, z_top, emissions_layer, emissions, i1, i2, j1, j2, km, grav)
    ...
    real(kind=RKIND), dimension(:,:,:), intent(in) :: delp
    real(kind=RKIND), dimension(:,:,:), intent(in) :: rhoa
    real(kind=RKIND), dimension(:,:),   intent(in) :: emissions_layer
    real(kind=RKIND), intent(in)                   :: z_bot
    real(kind=RKIND), intent(in)                   :: z_top
    real(kind=RKIND), dimension(:,:,:), intent(out):: emissions
    real(kind=RKIND), intent(in)                   :: grav
!   local
    integer :: i, j, k
    integer :: k_bot, k_top
    real(kind=RKIND) :: z_
    real(kind=RKIND), dimension(km) :: z, dz, w_
...
! !INOUTPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: aerosol_philic   ! OCphilic [kg kg-1]
   real(kind=RKIND), dimension(:,:), intent(inout), pointer :: aerosol_toHydrophilic ! OCHYPHIL [kg m-2 s-1]
...
! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in)                    :: UNDEF          ! set an undefined value (MAPL_UNDEF)
   real(kind=RKIND), intent(in)                    :: AVOGAD         ! Avogadro's number [1/kmol]
   real(kind=RKIND), intent(in)                    :: AIRMW          ! molecular weight of air [kg/kmol]
   real(kind=RKIND), intent(in)                    :: PI             ! pi constant
   real(kind=RKIND), intent(in)                    :: RUNIV          ! ideal gas constant [J/(Kmole*K)]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: rhoa           ! Layer air density [kg/m^3]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: tmpu           ! Layer temperature [K]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: relhum         ! relative humidity [1]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: delp           ! pressure thickness [Pa]
   real(kind=RKIND), pointer, dimension(:,:,:,:), intent(in) :: DU   ! dust aerosol [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:,:), intent(in) :: SS   ! sea salt aerosol [kg/kg]
   real(kind=RKIND), dimension(:) ,intent(in)      :: rmedDU         ! dust aerosol radius [um]
   real(kind=RKIND), dimension(:) ,intent(in)      :: rmedSS         ! sea salt aerosol radius [um]
   real(kind=RKIND), dimension(:) ,intent(in)      :: fnumDU         ! number of dust particles per kg mass
   real(kind=RKIND), dimension(:) ,intent(in)      :: fnumSS         ! number of sea salt particles per kg mass
   integer, intent(in)                             :: km             ! number of model levels
   integer, intent(in)                             :: klid           ! index for pressure lid
   real(kind=RKIND), intent(in)                    :: cdt            ! chemistry model timestep (sec)
   real(kind=RKIND), intent(in)                    :: grav           ! gravity (m/sec)
   real(kind=RKIND), intent(in)                    :: fMassHNO3      ! gram molecular weight
   real(kind=RKIND), intent(in)                    :: fMassNO3       ! gram molecular weight

! !INOUTPUT PARAMETERS:
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: NI_phet   ! Nitrate Production from Het Chem [kg/(m^2 sec)]
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: xhno3     ! buffer for NITRATE_HNO3 [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: HNO3_conc ! Nitric Acid Mass Concentration [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)    :: HNO3_sfcmass ! Nitric Acid Surface Mass Concentration [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)    :: HNO3_colmass ! Nitric Acid Column Mass Density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: nNO3an1 ! Nitrate bin 1 [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: nNO3an2 ! Nitrate bin 2 [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: nNO3an3 ! Nitrate bin 3 [kg/kg]

! !OUTPUT PARAMETERS:
   integer, optional, intent(out) :: rc

! !Local Variables
   real(kind=RKIND), dimension(:,:,:), allocatable :: kan1, kan2, kan3, kan
   real(kind=RKIND), dimension(:,:,:), allocatable :: deltahno3
...
! !INPUT PARAMETERS:
   ...
   integer, intent(in)                             :: km     ! model levels
   integer, intent(in)                             :: klid   ! index for pressure lid
   real(kind=RKIND), intent(in)                    :: rmed   ! aerosol radius [um]
   real(kind=RKIND), intent(in)                    :: fnum   ! number of aerosol particles per kg mass
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: rhoa   ! Layer air density [kg/m^3]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: temp   ! Layer temperature [K]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: rh     ! relative humidity [1]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: q      ! aerosol
   real(kind=RKIND), intent(in)                    :: AVOGAD ! Avogadro's number [1/kmol]
   real(kind=RKIND), intent(in)                    :: AIRMW  ! molecular weight of air [kg/kmol]
   real(kind=RKIND), intent(in)                    :: PI     ! pi constant
   real(kind=RKIND), intent(in)                    :: RUNIV  ! ideal gas constant [J/(Kmole*K)]
...
! !OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(out) :: kan

! !Local Variables
   integer :: i, j, k

   real(kind=RKIND) :: f_sad
   real(kind=RKIND) :: f_ad
...
! !USES:
   ...
   real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(inout) :: kan
   real(kind=RKIND), dimension(i1:i2,j1:j2,km), intent(in)    :: kan_total
!EOP
!------------------------------------------------------------------------------------
!  Begin..
...
! !INPUT PARAMETERS:
   real(kind=RKIND), intent(in) ::  tk   ! temperature [K]
   real(kind=RKIND), intent(in) ::  rh   ! fractional relative humidity [0 - 1]
   real(kind=RKIND), intent(in) ::  sad  ! aerosol surface area density [cm2 cm-3]
   real(kind=RKIND), intent(in) ::  ad   ! air number concentration [# cm-3]
   real(kind=RKIND), intent(in) ::  radA ! aerosol radius [cm]

   real(kind=RKIND)  :: pi   ! pi constant
   real(kind=RKIND)  :: rgas ! ideal gas constant [J/(K*mol)]
   real(kind=RKIND)  :: fMassHNO3 ! gram molecular weight of HNO3
   real(kind=RKIND) :: sktrs_hno3

! !Local Variables
   real(kind=RKIND), parameter :: fmassHNO3_hno3 = 63.013

!   REAL(kind=RKIND),  PARAMETER :: GAMMA_HNO3 = 0.1
   REAL(kind=RKIND),  PARAMETER :: GAMMA_HNO3 = 1.0e-3
!   REAL(kind=RKIND),  PARAMETER :: GAMMA_HNO3 = 5.0e-4

   real(kind=RKIND) :: dfkg
   real(kind=RKIND) :: avgvel
   real(kind=RKIND) :: gamma
   real(kind=RKIND) :: f_rh
   real(kind=RKIND) :: sqrt_tk
!   real(kind=DP) :: pi_dp = pi
!   real(kind=DP) :: rgas_dp = rgas

!   real(kind=RKIND), parameter :: p_dfkg   = sqrt(3.472e-2 + 1.0/fmassHNO3)
!   real(kind=RKIND), parameter :: p_avgvel = sqrt(8.0 * rgas_dp * 1000.0 / (pi_dp * fmassHNO3))

   real(kind=DP) :: pi_dp
   real(kind=DP) :: rgas_dp

   real(kind=RKIND) :: p_dfkg
   real(kind=RKIND) :: p_avgvel
...
! !INPUT PARAMETERS:
   real(kind=RKIND)  :: tk   ! temperature [K]
   real(kind=RKIND)  :: sad  ! aerosol surface area density [cm2 cm-3]
   real(kind=RKIND)  :: ad   ! air number concentration [# cm-3]
   real(kind=RKIND)  :: radA ! aerosol radius [cm]
   real(kind=RKIND)  :: sktrs_sslt
   real(kind=RKIND)  :: pi   ! pi constant
   real(kind=RKIND)  :: rgas ! ideal gas constant [J/(K*mol)]
   real(kind=RKIND)  :: fMassHNO3 ! gram molecular weight of HNO3
...
!  Locals
   REAL(kind=RKIND),  PARAMETER :: GAMMA_SSLT = 0.1e0
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: cdt, grav ! model time, and gravity respectively
   integer, intent(in) :: nymd, nhms
   real(kind=RKIND), intent(in)    :: fMassSO4  ! gram molecular weight of SO4
   real(kind=RKIND), intent(in)    :: fMassSO2  ! gram molecular weight of SO2
   real(kind=RKIND), intent(in)    :: fSO4ant   ! Fraction of anthropogenic emissions that are SO4
   ...
   real(kind=RKIND), intent(in)                 :: eAircraftFuel ! Aircraft emission factor: go from kg fuel to kg SO2
   ...
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: oro   ! orography flag
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: u10m  ! 10-m u-wind component [m s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: v10m  ! 10-m v-wind component [m s-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: hghte ! top of layer geopotential height [m]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: pblh
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: tmpu  ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: rhoa  ! Layer air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: delp  ! pressure thickness [Pa]
...
! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: so2, so4 ! Sulfate  internal state varaibles [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:)  :: SU_emis      ! SU emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)  :: SU_SO4eman  ! SO4 anthro emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)  :: SU_SO2eman  ! SO2 anthro emissions, kg/m2/s
   real(kind=RKIND), pointer, dimension(:,:)  :: SU_SO2embb  ! SO2 bioburn emissions, kg/m2/s
...
! !Local Variables
   ...
   real(kind=RKIND) :: p1, z1, dz, deltaz, deltap, f100, f500, fPblh
   real(kind=RKIND) :: zpbl
                          ! pressure at 100m, 500m, & PBLH
   real(kind=RKIND), dimension(:,:), allocatable :: p100, p500, pPblh, p0, z0, ps
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: cdt  ! model time step [seconds]
   real(kind=RKIND), intent(in)    :: grav ! gravity [m sec-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in)  :: tmpu  ! temperature [K]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: u10m  ! 10-m u-wind component [m s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: v10m  ! 10-m v-wind component [m s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)    :: oro   ! orography flag
   ...
   real(kind=RKIND), intent(in)    :: fMassDMS  ! gram molecular weight of DMS


! !INOUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: dms ! dms [kg kg-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: SU_emis   ! SU emissions, kg/m2/s
...
! !Local Variables
   ...
   real(kind=RKIND), dimension(:,:), allocatable :: srcDMS
   real(kind=RKIND) :: sCO2, schmidt, w10m, akw, sst
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:), intent(in)    :: vSO2   ! volcanic emission from file [kg]
   real(kind=RKIND), dimension(:), intent(in)    :: vCloud ! top elevation of emissions [m]
   integer, dimension(:), intent(in) :: iPoint, jPoint ! sub-domain locations of volcanos
   integer, intent(in) :: nhms ! current model time [sec]
   integer, intent(in) :: nSO2 ! index of SO2 relative to other sulfate tracers
   integer, intent(in) :: km   ! number of model levels
   real(kind=RKIND), intent(in)    :: cdt  ! model time step [sec]
   real(kind=RKIND), pointer, dimension(:,:,:) :: hghte     ! top of layer geopotential height [m]
   real(kind=RKIND), intent(in)    :: grav ! gravity [m sec-1]
!   real(kind=RKIND), dimension(:,:,:), intent(in) :: airdens ! layer air density [kg/m^3]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: delp    ! pressure thickness [Pa]
   real(kind=RKIND), dimension(:,:), intent(in)    :: area    ! area of grid cell [m^2]
   real(kind=RKIND), dimension(:), intent(in)      :: vLat    ! latitude specified in file [degree]
   real(kind=RKIND), dimension(:), intent(in)      :: vLon    ! longitude specified in file [degree]
! !INOUT PARAMETERS:
  real(kind=RKIND), pointer, dimension(:,:), intent(inout) :: SO2EMVN ! non-explosive volcanic emissions [kg m-2 s-1]
  real(kind=RKIND), pointer, dimension(:,:), intent(inout) :: SO2EMVE ! explosive volcanic emissions [kg m-2 s-1]
  real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: SO2 ! SO2 [kg kg-1]
  real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: SU_emis      ! SU emissions, kg/m2/s
  real(kind=RKIND), dimension(:), intent(inout) ::  vElev ! bottom elevation of emissions [m]
...
! !Local Variables
   ...
   real(kind=RKIND) :: so2volcano

   real(kind=RKIND) :: hup, hlow, dzvolc, dz, z1
   real(kind=RKIND) :: deltaSO2v
   real(kind=RKIND), dimension(:,:), allocatable :: z0
   real(kind=RKIND), allocatable, dimension(:,:) :: srcSO2volc
   real(kind=RKIND), allocatable, dimension(:,:) :: srcSO2volce
...
! !INPUT PARAMETERS:
   ...
   integer, intent(in)    :: nhms_current      ! current model NHMS
   integer, intent(inout) :: nymd_last         ! NYMD of last emission update
   integer, intent(in)    :: km                ! number of model levels
   real(kind=RKIND), intent(in) :: cdt         ! chemistry model time-step
   real(kind=RKIND), intent(in) :: undefval    ! value for undefined values
   real(kind=RKIND), intent(in) :: radToDeg    ! radian to degrees conversion
   real(kind=RKIND), intent(in) :: nAvogadro   ! Avogadro's number [molecules per mole of air]
   real(kind=RKIND), intent(in) :: pi          ! pi constant
   real(kind=RKIND), intent(in) :: airMolWght  ! air molecular weight [kg/Kmole]
   real(kind=RKIND), dimension(:,:), intent(in) :: lonRad, latRad ! model grid lon and lat
   real(kind=RKIND), dimension(:,:,:), intent(in) :: rhoa         ! layer air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: oh_clim, &   ! climatological OH
                                                                 no3_clim, &  ! climatological NO3
                                                                 h2o2_clim    ! climatological H2O2
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: xoh, xno3, xh2o2         ! returned oxidant values
...
! !Local Variables
   integer :: i, j, k, jday
   real(kind=RKIND)    :: qmax, xhour, xhouruse
   real(kind=RKIND), dimension(:,:), allocatable  :: cossza, sza
   real(kind=RKIND), dimension(:,:), allocatable  :: tcosz, tday, tnight
   integer :: n, ndystep
   integer :: i1=1, j1=1, i2, j2
...
! !INPUT PARAMETERS:
  ...
  real(kind=RKIND), intent(in)  :: xhour
  real(kind=RKIND), dimension(:,:), intent(in) :: lonRad, latRad   ! model grid lon and lat
  real(kind=RKIND), intent(in) :: PI, radToDeg
  real(kind=RKIND), dimension(:,:), intent(inout) :: cossza, sza
...
! !Local Variables
   ...
   real(kind=RKIND) :: a0, a1, a2, a3, b1, b2, b3, r, dec
   real(kind=RKIND) :: timloc, ahr, xlon, rlat
...
! !INPUT PARAMETERS:
   ...
   integer, intent(in) :: klid           ! index for pressure lid
   real(kind=RKIND), intent(in) :: cdt   ! chemisty model timestep
   logical, intent(in) :: KIN            ! true for aerosol
   real(kind=RKIND), intent(in) :: grav  ! gravity [m/sec]
   real(kind=RKIND), intent(in) :: airMolWght ! air molecular weight [kg]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: delp   ! pressure thickness [Pa]
   real(kind=RKIND), intent(in) :: fMassSO4, fMassSO2
   real(kind=RKIND), dimension(:,:,:) :: h2o2_int
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: ple     ! level edge air pressure
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rhoa    ! air density, [kg m-3]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: precc   ! total convective precip, [mm day-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: precl   ! total large-scale prec,  [mm day-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: pfllsan ! 3D flux of liquid nonconvective precipitation [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: pfilsan ! 3D flux of ice nonconvective precipitation [kg/(m^2 sec)]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: tmpu    ! temperature, [K]
   integer, intent(in) :: nDMS, nSO2, nSO4, nMSA !index position of sulfates
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: DMS ! [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: SO2 ! [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: SO4 ! [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: MSA ! [kg/kg]

! !OUTPUT PARAMETERS:
   real(kind=RKIND), pointer, dimension(:,:,:),intent(inout) :: fluxout
   real(kind=RKIND), pointer, dimension(:,:),intent(inout)  :: pSO4_colflux
   real(kind=RKIND), pointer, dimension(:,:),intent(inout)  :: pSO4wet_colflux
   real(kind=RKIND), pointer, dimension(:,:,:),intent(inout) :: pso4
   real(kind=RKIND), pointer, dimension(:,:,:),intent(inout) :: pso4wet
   integer, optional, intent(out)   :: rc    ! Error return code:
                                            !  0 - all is well
...
!   real(kind=RKIND) :: pdog(i1:i2,j1:j2,km)              ! air mass factor dp/g [kg m-2]
   real(kind=RKIND), dimension(:,:,:), allocatable :: pdog ! air mass factor dp/g [kg m-2]
   real(kind=DP) :: Td_ls, Td_cv              ! ls and cv timescales [s]
   real(kind=DP) :: pls, pcv, pac             ! ls, cv, tot precip [mm day-1]
   real(kind=DP) :: qls(km), qcv(km)          ! ls, cv portion of moisture tendency [kg m-3 s-1]
   real(kind=DP) :: qmx, qd, A                ! temporary variables on moisture
   real(kind=DP) :: F, B, BT                  ! temporary variables on cloud, freq.
   real(kind=DP), allocatable :: fd(:,:)      ! flux across layers [kg m-2]
   real(kind=DP), allocatable :: dpfli(:,:,:) !
   real(kind=DP), allocatable :: DC(:)        ! scavenge change in mass mixing ratio
!   real(kind=RKIND) :: c_h2o(i1:i2,j1:j2,km), cldliq(i1:i2,j1:j2,km), cldice(i1:i2,j1:j2,km)
   real(kind=RKIND), dimension(:,:,:), allocatable :: c_h2o, cldliq, cldice
   real(kind=RKIND), parameter :: kb = 1.3807e-23 ! Boltzmann constant [kg m2 s-1 K-1 mol-1]
...
!  Rain parameters (from where?)
   ...
   real(kind=RKIND), parameter :: F0_cv = 0.3
   real(kind=RKIND), parameter :: XL_cv = 2.0e-3
   real(kind=RKIND), parameter :: one = 1.0, zero = 0.0
...
!--- Begin LDF: pfllsan and pfilsan are already defined as flux divergence.
!  dpfli = pfllsan(:,:,1:km)-pfllsan(:,:,0:km-1)+pfilsan(:,:,1:km)-pfilsan(:,:,0:km-1)
   dpfli = pfllsan(:,:,:) + pfilsan(:,:,:)
...
!      What is the soluble amount of SO2?
       SO2Soluble = min(real(fmr*SO2(i,j,k), kind=DP),real(h2o2_int(i,j,k)*one, kind=DP))/fmr
...
!       Adjust H2O2 concentration in cloudy portion of cell
        if(fmr*SO2(i,j,k) .gt. h2o2_int(i,j,k)) then
         h2o2_int(i,j,k) = max(real(zero,kind=RKIND),real((one-F)*h2o2_int(i,j,k),kind=RKIND))
!  GOCART removes all
...
!         Adjust the flux out of the bottom of the layer--remove SO2 here!
   ...
   contains
     ...
     implicit NONE
     ! !INPUT PARAMETERS:
      real(kind=RKIND), intent(inout) :: aerosol
...
! !INPUT PARAMETERS:
   integer, intent(in) :: km    ! number of model levels
   integer,    intent(in)    :: klid   ! index for pressure lid
   real(kind=RKIND), intent(in)    :: rmed  ! mean radius [um]
   real(kind=RKIND), intent(in)    :: sigma ! Sigma of lognormal number distribution
   real(kind=RKIND), intent(in)    :: rhop  ! dry particle density [kg m-3]
   real(kind=RKIND), intent(in)    :: grav  ! gravity [m/sec]
   real(kind=RKIND), intent(in)    :: pi    ! pi constant
   ...
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: tropp ! tropopause pressure [Pa]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rh      ! relative humidity [1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: u       ! east-west wind [m s-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: v       ! north-south wind [m s-1]

! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: DMS  ! dimethyl sulfide [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: SO2  ! sulfer dioxide [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: SO4  ! sulfate aerosol [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout)  :: MSA  ! methanesulphonic acid [kg/kg]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: dmssfcmass ! sfc mass concentration [kg/m3]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: dmscolmass ! col mass density [kg/m2]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: msasfcmass ! sfc mass concentration [kg/m3]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: msacolmass ! col mass density [kg/m2]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: so2sfcmass ! sfc mass concentration [kg/m3]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: so2colmass ! col mass density [kg/m2]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: so4sfcmass ! sfc mass concentration [kg/m3]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: so4colmass ! col mass density [kg/m2]
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: exttau     ! ext. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: stexttau   ! Stratosphere ext. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: scatau     ! sct. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: stscatau   ! Stratosphere sct. AOT at 550 nm
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: so4mass    ! 3D sulfate mass mr
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: so4conc    ! 3D mass concentration, [kg/m3]
   real(kind=RKIND), optional, dimension(:,:,:,:), intent(inout)  :: extcoef    ! 3D ext. coefficient, [1/m]
   real(kind=RKIND), optional, dimension(:,:,:,:), intent(inout)  :: scacoef    ! 3D scat.coefficient, [1/m]
   real(kind=RKIND), optional, dimension(:,:,:,:), intent(inout)  :: bckcoef    ! 3D backscatter coefficient, [m-1 sr-1]
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: angstrom   ! 470-870 nm Angstrom parameter
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: fluxu      ! Column mass flux in x direction
   real(kind=RKIND), optional, dimension(:,:),   intent(inout)  :: fluxv      ! Column mass flux in y direction
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: sarea      ! Sulfate surface area density [m2 m-3]
   real(kind=RKIND), optional, dimension(:,:,:), intent(inout)  :: snum       ! Sulfate number density [# m-2]
   integer, optional, intent(out)   :: rc         ! Error return code:
                                                  !  0 - all is well
                                                  !  1 -
...
! !Local Variables
   ...
   real(kind=RKIND), dimension(:,:,:), allocatable :: tau, ssa, bck
   real(kind=RKIND), dimension(:,:), allocatable :: tau470, tau870
   integer    :: ilam470, ilam870
   logical :: do_angstrom
   real(kind=RKIND) :: rh_, gf, rwet, svol
...
! !INPUT PARAMETERS:
   integer, intent(in) :: km              ! number of model levels
   integer, intent(in) :: klid            ! index for pressure lid
   real(kind=RKIND), intent(in) :: cdt    ! chemisty model timestep [sec]
   real(kind=RKIND), intent(in) :: PI     ! pi constnat
   real(kind=RKIND), intent(in) :: radToDeg   ! radians to degree conversion
   real(kind=RKIND), intent(in) :: von_karman ! Von Karman constant [unitless]
   real(kind=RKIND), intent(in) :: nAvogadro  ! Avogadro's number [1/kmol]
   real(kind=RKIND), intent(in) :: airMolWght ! molecular weight of air [kg/kmol]
   real(kind=RKIND), intent(in) :: cpd
   real(kind=RKIND), intent(in) :: grav       ! gravity [m/sec]
   real(kind=RKIND), intent(in) :: fMassMSA, fMassDMS, fMassSO2, fMassSO4 ! gram molecular weights of species
   integer, intent(in) :: nymd   ! model year month day
   integer, intent(in) :: nhms   ! model hour mintue second
   real(kind=RKIND), dimension(:,:), intent(in) :: lonRad    ! model grid lon [radians]
   real(kind=RKIND), dimension(:,:), intent(in) :: latRad    ! model grid lat [radians]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: dms  ! dimethyl sulfide [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: so2  ! sulfer dioxide [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: so4  ! sulfate aerosol [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: msa  ! methanesulphonic acid [kg/kg]
   ...
   real(kind=RKIND), dimension(:,:,:), intent(in) :: delp    ! pressure thickness [Pa]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: tmpu   ! temperature [K]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: cloud  ! cloud fraction for radiation [1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: rhoa   ! layer air density [kg/m^3]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(in) :: hghte  ! top of layer geopotential height [m]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: ustar  ! surface velocity scale [m/sec]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: shflux ! sensible heat flux from turbulence [w/m^2]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: oro    ! land-ocean-ice mask
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: pblh   ! planetary boundary layer height [m]
   real(kind=RKIND), pointer, dimension(:,:), intent(in)   :: z0h    ! surface roughness for heat [m]

! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: xoh, xno3, xh2o2 ! OH, NO3, H2O2 respectievly [kg/kg]
   real(kind=RKIND), dimension(:,:,:) :: h2o2_init ! private H2O2 that is saved and used to initialize [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: SU_dep ! Sulfate Dry Deposition All Bins [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)   :: SU_PSO2 ! SO2 Prod from DMS Oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)   :: SU_PMSA ! MSA Prod from DMS Oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)   :: SU_PSO4 ! SO4 Prod from All SO2 Oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)   :: SU_PSO4g ! SO4 Prod from Gaseous SO2 Oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout)   :: SU_PSO4aq ! SO4 Prod from Aqueous SO2 Oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: pso2 ! SO2 Prod from DMS oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: pmsa ! MSA Prod from DMS oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: pso4 ! SO4 Prod from all SO2 oxidation [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: pso4g ! SO4 Prod from gaseous SO2 oxidation [kg m-2 s-1]
...
! !Local Variables
   ...
   real(kind=RKIND), dimension(:,:,:), allocatable :: pSO2_DMS, pMSA_DMS, pSO4g_SO2, pSO4aq_SO2
   real(kind=RKIND)    :: xhour
   integer :: status
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: airMolWght ! molecular weight of air [kg/kmol]
   real(kind=RKIND), intent(in)    :: cpd
   real(kind=RKIND), intent(in)    :: fMassMSA, fMassDMS, fMassSO2 ! gram molecular weights of species
   integer, intent(in) :: nDMS       !index position of sulfates
   real(kind=RKIND), dimension(:,:,:), intent(in) :: xoh, xno3  ! OH, NO3 respectievly [kg/kg]
   real(kind=RKIND), dimension(:,:), intent(in)   :: cossza
   real(kind=RKIND), dimension(:,:,:), intent(in) :: tmpu   ! temperature [K]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: rhoa   ! layer air density [kg/m^3]


! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: qa  ! dimethyl sulfide [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: SU_dep ! Sulfate Dry Deposition All Bins [kg m-2 s-1]

! !OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), allocatable,  intent(out) :: pSO2_DMS ! SO2 production from DMS oxidation [kg kg-1 s-1]
   real(kind=RKIND), dimension(:,:,:), allocatable,  intent(out) :: pMSA_DMS ! MSA production from DMS oxidation [kg kg-1 s-1]
   integer, optional, intent(out)   :: rc
...
! !Local Variables
   ...
   real(kind=DP) :: Fx, b, eff
   real(kind=DP) :: rk1, rk2, rk3, rk4
   real(kind=DP) :: tk, o2, oh, no3, air
   real(kind=DP) :: dms, dms0, dms_oh
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)    :: cpd
   real(kind=RKIND), intent(in)    :: grav       ! gravity [m/sec]
   real(kind=RKIND), intent(in)    :: fMassSO4, fMassSO2 ! gram molecular weights of species
   integer, intent(in) :: nSO2       !index position of sulfates
   real(kind=RKIND), dimension(:,:,:), intent(in) :: tmpu   ! temperature [K]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: rhoa   ! layer air density [kg/m^3]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: delp   ! pressure thickness [Pa]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: cloud  ! cloud fraction for radiation [1]
   real(kind=RKIND), dimension(:,:), intent(in)   :: drydepf  ! dry deposition frequency [s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(in) :: oro  ! land-ocean-ice mask
   real(kind=RKIND), dimension(:,:,:), intent(in) :: pSO2_DMS ! SO2 production from DMS oxidation [kg m-2 s-1]

! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: qa  ! dimethyl sulfide [kg/kg]
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: xoh, xh2o2  ! OH, H2O2 respectievly [kg/kg]
   real(kind=RKIND), pointer, dimension(:,:,:), intent(inout) :: SU_dep ! Sulfate Dry Deposition All Bins [kg m-2 s-1]

! !OUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), allocatable, intent(out) :: pSO4g_SO2 ! SO4 production - gas phase [kg kg-1 s-1]
   real(kind=RKIND), dimension(:,:,:), allocatable, intent(out) :: pSO4aq_SO2 ! SO4 production - aqueous [kg kg-1 s-1]
   integer, optional, intent(out)   :: rc
...
! !Local Variables
   integer :: i, j, k, j2, i2
   real(kind=DP) :: rk1, rk2, rk, rkt, f1
   real(kind=DP) :: L1, L2, Ld, SO2, SO2_cd, fc, fMR
   real(kind=DP) :: oh, h2o2, SO20, tk, air, k0, ki, kk
   real(kind=RKIND), dimension(:,:), allocatable :: fout
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:,:,:), intent(in) :: delp   ! pressure thickness [Pa]
   real(kind=RKIND), dimension(:,:), intent(in)   :: drydepf    ! dry deposition frequency [s-1]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: pSO4g_SO2  ! SO4 production - gas phase [kg kg-1 s-1]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: pSO4aq_SO2 ! SO4 production - aqueous [kg kg-1 s-1]
...
! !Local Variables
   integer :: i, j, k, i2, j2
   real(kind=DP) :: rk, rkt, Ld
   real(kind=DP) :: SO4, SO40, pSO4
   real(kind=RKIND), dimension(:,:), allocatable :: fout
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), dimension(:,:,:), intent(in) :: delp   ! pressure thickness [Pa]
   real(kind=RKIND), dimension(:,:), intent(in)   :: drydepf   ! dry deposition frequency [s-1]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: pMSA_DMS  ! MSA production - gas phase [kg kg-1 s-1]
...
! !OUTPUT PARAMETERS:
   real(kind=RKIND), intent(out):: c1,c2,c3,c4
   integer, optional, intent(out) :: rc                   ! Error return code:
...
! !Local Variables
  ...
  REAL(kind=RKIND) ,PARAMETER :: notfound = -1.
...
!     * indica artigos nao encontrados nesse endereço eletronico
  REAL(kind=RKIND),PARAMETER,DIMENSION(nspecies_HL) :: hstar=(/&
    1.10E-2              ,   & ! O3 - 001
    8.30E+4              ,   & ! H2O2 - 002
    1.90E-3              ,   & ! NO - 003
    1.20E-2              ,   & ! NO2 - 004
...
!     Environmental Chemistry (Version 3)
!     http://www.henrys-law.org
  REAL(kind=RKIND),PARAMETER,DIMENSION(nspecies_HL) :: dhr=(/&
    2400.         ,   & ! O3 - 001
    7400.         ,   & ! H2O2 - 002
    1400.         ,   & ! NO - 003
    2500.         ,   & ! NO2 - 004
  ...
  REAL(kind=RKIND),PARAMETER,DIMENSION(nspecies_HL) :: weight=(/&
    48.  ,   & ! O3 - 001
    34.  ,   & ! H2O2 - 002
    30.  ,   & ! NO - 003
    46.  ,   & ! NO2 - 004
...
!     vol1-4 Plenum Press New York
  REAL(kind=RKIND),PARAMETER,DIMENSION(nspecies_HL) :: ak0=(/&
    0.00E+00     ,   & ! O3 - 001
    2.20E-12     ,   & ! H2O2 - 002
    0.00E+00     ,   & ! NO - 003
    0.00E+00     ,   & ! NO2 - 004
...
!     [K]
!     Referencias: Barth et al. JGR 112, D13310 2007
  REAL(kind=RKIND),PARAMETER,DIMENSION(nspecies_HL) :: dak=(/&
    0.         ,   & ! O3 - 001
    -3700.     ,   & ! H2O2 - 002
    0.         ,   & ! NO - 003
    0.         ,   & ! NO2 - 004
...
! !INPUT PARAMETERS:
   integer, intent(in) :: km    ! total model levels
   integer, intent(in) :: klid  ! index for pressure lid
   real(kind=RKIND), intent(in)    :: cdt   ! model time step [s]
   real(kind=RKIND), intent(in)    :: grav  ! gravity [m s-2]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: delp   ! pressure thickness [Pa]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: rhoa   ! Layer air density [kg m-3]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: tmpu   ! Layer temperature [K]
   real(kind=RKIND), dimension(:,:,:), intent(in)  :: rh     ! relative humidity [0-1]
   real(kind=RKIND), intent(in)  :: fMassHNO3   ! gram molecular weight of HNO3
   real(kind=RKIND), intent(in)  :: fMassAir    ! gram molecular weight of air

! !INOUTPUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: SO4    ! Sulphate aerosol [kg kg-1]
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: NH3    ! Ammonia (NH3, gas phase) [kg kg-1]
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: NO3an1 ! Nitrate size bin 001 [kg kg-1]
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: NH4a   ! Ammonium ion (NH4+, aerosol phase) [kg kg-1]
   real(kind=RKIND), dimension(:,:,:), intent(inout)  :: xhno3  ! buffer for NITRATE_HNO3 [kg m-2 sec-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout) :: NI_pno3aq ! Nitrate Production from Aqueous Chemistry [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout) :: NI_pnh4aq ! Ammonium Production from Aqueous Chemistry [kg m-2 s-1]
   real(kind=RKIND), pointer, dimension(:,:), intent(inout) :: NI_pnh3aq ! Ammonia Change from Aqueous Chemistry [kg m-2 s-1]
...
!     do not update on return.
      SO4_  = 1.d-32
      SO4_  = max(1.d-32,real(SO4(i,j,k) * fmmr_to_conc, kind=DP))
      GNO3  = max(1.d-32,real(xhno3(i,j,k) * fMassHNO3 / fMassAir * fmmr_to_conc, kind=DP))
      GNH3  = max(1.d-32,real(NH3(i,j,k)  * fmmr_to_conc, kind=DP))
      RH_   = rh(i,j,k)
      TEMP  = tmpu(i,j,k)
      ASO4  = 1.d-32
      AHSO4 = 1.d-32
      ANO3  = max(1.d-32,real(NO3an1(i,j,k) * fmmr_to_conc, kind=DP))
      AH2O  = 1.d-32
      ANH4  = max(1.d-32,real(NH4a(i,j,k) * fmmr_to_conc, kind=DP))
...
!                          as REAL(kind=DP)'s.  Cleaned up comments.  Also now force
...
!
! *** Modified 09/25/2002
!     Ported into "rpmares_mod.f".  Now declare all variables with REAL(kind=DP).
...
!
      ! Arguments
      INTEGER       :: IRHX
      REAL(kind=DP) :: MSO4, MNH4, MNO3, WH2O

      ! Local variables
      INTEGER       :: IRH
      REAL(kind=DP) :: TSO4,  TNH4,  TNO3,  X,      AW,     AWC
      REAL(kind=DP) :: MFS0,  MFS1,  MFS15, Y
      REAL(kind=DP) :: Y0,    Y1,    Y15,   Y2,     Y3,     Y40
      REAL(kind=DP) :: Y140,  Y1540, YC,    MFSSO4, MFSNO3

      ! Molecular weight parameters
      REAL(kind=DP), PARAMETER :: MWSO4  = 96.0636d0
      REAL(kind=DP), PARAMETER :: MWNH4  = 18.0985d0
      REAL(kind=DP), PARAMETER :: MWNO3  = 62.0649d0
      REAL(kind=DP), PARAMETER :: MW2    = MWSO4 + 2.0d0 * MWNH4
      REAL(kind=DP), PARAMETER :: MWANO3 = MWNO3 + MWNH4
      ...
      !=================================================================
      REAL(kind=DP) :: C1(4)  = (/ 0.9995178d0,  -0.7952896d0, &
     &                             0.99683673d0, -1.143874d0 /)

      REAL(kind=DP) :: C15(4) = (/ 1.697092d0, -4.045936d0, &
     &                             5.833688d0, -3.463783d0 /)
      ...
      !=================================================================
      REAL(kind=DP) :: C0(4)  =  (/ 0.798079d0, -1.574367d0, &
     &                              2.536686d0, -1.735297d0 /)
      ...
      !=================================================================
      REAL(kind=DP) :: KNO3(6) = (/  0.2906d0,   6.83665d0, -26.9093d0, &
     &                              46.6983d0, -38.803d0,    11.8837d0 /)

      REAL(kind=DP) :: KSO4(6) = (/   2.27515d0, -11.147d0,   36.3369d0, &
...
!     recommendations for setting a0, a1,a2 as real(kind=DP) variables.
!
...
!
      ! Arguments
      INTEGER       :: NR
      REAL(kind=DP) :: A2, A1, A0
      REAL(kind=DP) :: CRUTES(3)

      integer, intent(out) :: rc

      ! Local variables
      REAL(kind=DP)            :: QQ,    RR,    A2SQ,  THETA, DUM1, DUM2
      REAL(kind=DP)            :: PART1, PART2, PART3, RRSQ,  PHI,  YY1
      REAL(kind=DP)            :: YY2,   YY3,   COSTH, SINTH
      REAL(kind=DP), PARAMETER :: ONE    = 1.0d0
      REAL(kind=DP), PARAMETER :: SQRT3  = 1.732050808d0
      REAL(kind=DP), PARAMETER :: ONE3RD = 0.333333333d0
      ! !LOCAL VARIABLES:
      character (len=75) :: err_msg
...
!                         declaring REALs as REAL(kind=DP) and by using "D" exponents.
!******************************************************************************
      ! Error codes
      ...
      REAL(kind=DP),  PARAMETER :: XSTAT0 = 0       ! Normal, successful completion
      REAL(kind=DP),  PARAMETER :: XSTAT1 = 1       ! File I/O error
      REAL(kind=DP),  PARAMETER :: XSTAT2 = 2       ! Execution error
      REAL(kind=DP),  PARAMETER :: XSTAT3 = 3       ! Special  error

      !=================================================================
      ! ARGUMENTS and their descriptions
      !=================================================================
      REAL(kind=DP), optional   :: MOLNU            ! tot # moles of all ions
      REAL(kind=DP), optional   :: PHIMULT          ! multicomponent paractical
                                                    !   osmotic coef
      REAL(kind=DP)             :: CAT(NCAT)        ! cation conc in moles/kg (input)
      REAL(kind=DP)             :: AN(NAN)          ! anion conc in moles/kg (input)
      REAL(kind=DP)             :: GAMA(NCAT,NAN)   ! mean molal ionic activity coefs
      ...
      REAL(kind=DP)             :: TEXPV            !
      REAL(kind=DP)             :: TRM              !
      REAL(kind=DP)             :: TWOI             ! 2*ionic strength
      REAL(kind=DP)             :: TWOSRI           ! 2*sqrt of ionic strength
      REAL(kind=DP)             :: ZBAR             !
      REAL(kind=DP)             :: ZBAR2            !
      REAL(kind=DP)             :: ZOT1             !
      REAL(kind=DP)             :: SRI              ! square root of ionic strength
      REAL(kind=DP)             :: F2(NCAT)         !
      REAL(kind=DP)             :: F1(NAN)          !
      REAL(kind=DP)             :: BGAMA (NCAT,NAN) !
      REAL(kind=DP)             :: X     (NCAT,NAN) !
      REAL(kind=DP)             :: M     (NCAT,NAN) ! molality of each electrolyte
      REAL(kind=DP)             :: LGAMA0(NCAT,NAN) ! binary activity coefficients
      REAL(kind=DP)             :: Y     (NAN,NCAT) !
      REAL(kind=DP)             :: BETA0 (NCAT,NAN) ! binary activity coef parameter
      REAL(kind=DP)             :: BETA1 (NCAT,NAN) ! binary activity coef parameter
      REAL(kind=DP)             :: CGAMA (NCAT,NAN) ! binary activity coef parameter
      REAL(kind=DP)             :: V1    (NCAT,NAN) ! # of cations in electrolyte
                                                    !   formula
      REAL(kind=DP)             :: V2    (NCAT,NAN) ! # of anions in electrolyte
                                                    !   formula
      ! absolute value of charges of cation
      REAL(kind=DP)             :: ZP(NCAT) = (/ 1.0d0, 1.0d0 /)

      ! absolute value of charges of anion
      REAL(kind=DP)             :: ZM(NAN)  = (/ 2.0d0, 1.0d0, 1.0d0 /)
...
!!     err_real2     : real 2 to print out
      ...
      real(kind=DP)    , intent(in) :: err_real1
      real(kind=DP)    , intent(in) :: err_real2
...
! !USES:
   ...
   real(kind=RKIND) :: val                    ! resolution dependent value

! !INPUT/OUTPUT PARAMETERS:
   integer, intent(in) :: im_World, jm_World  ! number of global grid cells
   real(kind=RKIND), intent(in) :: res_value(:) ! array with the resolution dependent values:
...
! !ARGUMENTS:
       ...
       real(kind=RKIND), intent(out)   :: Eout(:,:) ! Emissions valid at NHMS
       real(kind=RKIND), intent(in)    :: Ein(:,:)  ! Daily-mean emissions
       real(kind=RKIND), intent(in)    :: lons(:,:) ! Latitudes in degrees
       real(kind=RKIND), intent(in)    :: lats(:,:) ! Latitudes in degrees
       integer, intent(in) :: nhms
       real(kind=RKIND), intent(in)    :: cdt       ! time step in seconds
...
!      -------------------------------------------
       ...
       real(kind=RKIND),    parameter :: DT = 86400. / N
...
!      optical depth during the day
       real(kind=RKIND),    parameter :: Boreal(N) = 1.0
!      real(kind=RKIND),    parameter :: Boreal(N) = &
...
!         0.1028, 0.0956, 0.0886, 0.0818, 0.0751, 0.0687 /)
       real(kind=RKIND),    parameter :: NonBoreal(N) = &
...
!      ---------------------------------------------------------------------
       real(kind=DP) :: fBoreal, fNonBoreal
       real(kind=RKIND) :: fDT

       integer :: hh, mm, ss, ndt, i, j, k
       integer :: NN
       real(kind=RKIND) :: secs, secs_local, aBoreal, aNonBoreal, alpha
...
!==================================================================================
   ...
   subroutine ReadPointEmissions( nymd, filename, nPts, vLat, vLon, vBase, vTop, vEmis, vStart, vEnd, unusable, label, rc)
      ...
      real(kind=RKIND), allocatable, dimension(:), intent(out)    :: vLat, vLon, vTop, vBase, vEmis
      ...
      real(kind=RKIND), allocatable :: table(:,:)
      integer :: nCols
