...
!--- types needed to define CA2G_oc:
 ...
 type,extends(GA_Environment),public:: CA2G_oc_GridComp
    ...
    contains
       ...
       procedure:: rrtmg_GridComp     => rrtmg_CA2G_oc_GridComp
...
!=================================================================================================================
 subroutine processes_CA2G_oc_GridComp(self_params,self,to_MYNN,to_THOM,its,ite,jts,jte,kts,kte)
!=================================================================================================================
...
!    we simply skip this step:
 if(associated(self%ocvdep)) self%ocvdep(:,:) = drydepf(:,:)*self%delz(:,:,self_params%km)
 if(.not. to_MYNN) then
    do ibin = 1, self_params%nbins
       dqa = 0.
...
!=================================================================================================================
 subroutine rrtmg_CA2G_oc_GridComp(self_params,self,its,ite,jts,jte,kts,kte,nbndlw,nbndsw)
!==================================================================================================================
...
!------------------------------------------------------------------------------------------------------------------
!call mpas_log_write(' ')
 call mpas_log_write('--- enter subroutine rrtmg_CA2G_oc_GridComp:')
...
!call mpas_log_write('--- nbands = $i',intArgs=(/nbands/))
 ...
 if(associated(self%octau_sw)) self%octau_sw(:,:,:,:) = 0._RKIND
 if(associated(self%ocasy_sw)) self%ocasy_sw(:,:,:,:) = 0._RKIND
 if(associated(self%ocssa_sw)) self%ocssa_sw(:,:,:,:) = 0._RKIND
 if(associated(self%octau_lw)) self%octau_lw(:,:,:,:) = 0._RKIND
 if(associated(self%ocasy_lw)) self%ocasy_lw(:,:,:,:) = 0._RKIND
 if(associated(self%ocssa_lw)) self%ocssa_lw(:,:,:,:) = 0._RKIND
 ...
 qca2G(:,:,:,1) = self%ocphobic(:,:,:)*self%delp(:,:,:)/grav
 qca2G(:,:,:,2) = self%ocphilic(:,:,:)*self%delp(:,:,:)/grav
 ...
 do n = 1, nbands
    ...
    if(istat /=0) then
       call mpas_log_write('--- CA2G_oc_GridComp: error in subroutine rrtmg_CA2G_oc_GridComp.', &
                           messageType=MPAS_LOG_CRIT)
    else
       if(n .le. nbndsw) then
          ns = ns+1
          self%octau_sw(:,:,:,ns) = real(ext_s(:,:,:),kind=RKIND)
          self%ocasy_sw(:,:,:,ns) = real(asy_s(:,:,:),kind=RKIND)
          self%ocssa_sw(:,:,:,ns) = real(ssa_s(:,:,:),kind=RKIND)
       elseif(n .gt. nbndsw) then
          nl = nl+1
          self%octau_lw(:,:,:,nl) = real(ext_s(:,:,:),kind=RKIND)
          self%ocasy_lw(:,:,:,nl) = real(asy_s(:,:,:),kind=RKIND)
          self%ocssa_lw(:,:,:,nl) = real(ssa_s(:,:,:),kind=RKIND)
       endif
    endif
 enddo
 ...
 call mpas_log_write('--- end subroutine rrtmg_CA2G_oc_GridComp.')
 ...
 end subroutine rrtmg_CA2G_oc_GridComp
