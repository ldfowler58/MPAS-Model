...
! !INOUT PARAMETERS:
   real(kind=RKIND), dimension(:,:,:), intent(inout) :: int_ptr ! aerosol pointer
...
! !INPUT PARAMETERS:
   ...
   real(kind=RKIND), intent(in)       :: plid ! pressure lid [hPa]
   real(kind=RKIND), dimension(:,:,:), intent(in) :: ple  ! air pressure [Pa]
...
! !Local Variables
   integer :: k, kk, j, i
   real(kind=RKIND) :: plid_, diff, refDiff
   real(kind=RKIND), allocatable, dimension(:) :: pres  ! pressure at each model level [Pa]
