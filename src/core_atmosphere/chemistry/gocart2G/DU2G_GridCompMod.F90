...
!==================================================================================================================
 ...
 use DU2G_instance,only: nbins,particle_radius_microns,particle_density,fscav,molecular_weight,          &
                         ...
                         radius_lower,radius_upper,particle_radius_number,sigma,source_fraction,ipoint,  &
                         Ch_DU
...
!--- types needed to define DU2G:
 ...
 type,extends(GA_Environment),public:: DU2G_GridComp
    ...
    contains
       ...
       procedure:: rrtmg_GridComp     => rrtmg_DU2G_GridComp
...
!------------------------------------------------------------------------------------------------------------------
 call mpas_log_write(' ')
 call mpas_log_write('--- enter subroutine emissions_DU2G_GridComp:')
...
!------------------------------------------------------------------------------------------------------------------
!call mpas_log_write(' ')
 call mpas_log_write('--- enter subroutine processes_DU2G_GridComp:')
...
!==================================================================================================================
 subroutine rrtmg_DU2G_GridComp(self_params,self,its,ite,jts,jte,kts,kte,nbndlw,nbndsw)
!==================================================================================================================
...
!--- local variables:
 ...
 real(kind=RKIND),dimension(:,:,:,:),allocatable:: qdu5G

!------------------------------------------------------------------------------------------------------------------
!call mpas_log_write(' ')
 call mpas_log_write('--- enter subroutine rrtmg_DU2G_GridComp:')
...
!call mpas_log_write('--- nbands = $i',intArgs=(/nbands/))
 ...
 if(associated(self%dutau_sw)) self%dutau_sw(:,:,:,:) = 0._RKIND
 if(associated(self%duasy_sw)) self%duasy_sw(:,:,:,:) = 0._RKIND
 if(associated(self%dussa_sw)) self%dussa_sw(:,:,:,:) = 0._RKIND
 if(associated(self%dutau_lw)) self%dutau_lw(:,:,:,:) = 0._RKIND
 if(associated(self%duasy_lw)) self%duasy_lw(:,:,:,:) = 0._RKIND
 if(associated(self%dussa_lw)) self%dussa_lw(:,:,:,:) = 0._RKIND
 ...
 if(.not.allocated(qdu5G)) allocate(qdu5G(its:ite,jts:jte,kts:kte,nbins))
 ...
 do n = 1,nbins
    qdu5G(:,:,:,n) = self%du(:,:,:,n)*self%delp(:,:,:)/grav
 enddo
 ...
 do n = 1, nbands
    ...
    ext_s(:,:,:) = 0._RKIND
    ssa_s(:,:,:) = 0._RKIND
    call mie_(self_params%rad_Mie,its,ite,jts,jte,kts,kte,nbins,n,qdu5G,self%rh2,ext_s,ssa_s,asy_s,istat)
    if(istat /=0) then
       call mpas_log_write('--- DU2G_GridComp: error in subroutine rrtmg_DU2G_GridComp.', &
                           messageType=MPAS_LOG_CRIT)
    else
       if(n .le. nbndsw) then
          ns = ns+1
          self%dutau_sw(:,:,:,ns) = real(ext_s(:,:,:),kind=RKIND)
          self%duasy_sw(:,:,:,ns) = real(asy_s(:,:,:),kind=RKIND)
          self%dussa_sw(:,:,:,ns) = real(ssa_s(:,:,:),kind=RKIND)
       elseif(n .gt. nbndsw) then
          nl = nl+1
          self%dutau_lw(:,:,:,nl) = real(ext_s(:,:,:),kind=RKIND)
          self%duasy_lw(:,:,:,nl) = real(asy_s(:,:,:),kind=RKIND)
          self%dussa_lw(:,:,:,nl) = real(ssa_s(:,:,:),kind=RKIND)
       endif
    endif
 enddo
 ...
 if(allocated(qdu5G)) deallocate(qdu5G)

 call mpas_log_write('--- end subroutine rrtmg_DU2G_GridComp.')
 ...
 end subroutine rrtmg_DU2G_GridComp
